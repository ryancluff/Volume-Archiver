...
def backupjob_created(spec, name, namespace, logger, **kwargs):
    ...
    change_state(name, namespace, "job created")
...
@kopf.on.event('batch', 'v1', 'jobs', field='status.succeeded', value=1, annotations={'volumearchiver.rcluff.com/backupjob': kopf.PRESENT})
def job_succeeded(name, namespace, meta, **_):
    apps_api = kubernetes.client.AppsV1Api()
    cobject_api = kubernetes.client.CustomObjectsApi()
